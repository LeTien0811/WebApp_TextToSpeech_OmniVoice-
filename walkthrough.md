...
## 🛠️ Bản vá sửa lỗi (Bug Fixes - 27/06/2026)

Hệ thống đã được khắc phục hai lỗi quan trọng liên quan đến đường dẫn tĩnh (404 Not Found) và chất lượng âm thanh sinh ra (nuốt từ đầu tiên).

### 1. Sửa lỗi 404 Not Found khi tải/phát tệp âm thanh đầu ra
* **Nguyên nhân**: FastAPI mount thư mục `STATIC_DIR` lên đường dẫn gốc `/`, dẫn đến việc các tệp tin xuất ra tại `static/outputs/` được truy cập thông qua `/outputs/{task_id}.mp3`. Tuy nhiên, luồng API SSE lại trả về đường dẫn `/static/outputs/{task_id}.mp3`. Vì không có endpoint hay mount nào bắt đầu bằng `/static`, máy chủ trả về mã lỗi 404.
* **Khắc phục**: 
  * Cập nhật [main.py](file:///e:/Document/TextToSpeech_Project/OmniVoice_TTS_Service_api/main.py) để mount thêm đường dẫn tĩnh `/static` trỏ tới `STATIC_DIR` trước khi mount gốc `/`.
  * Giờ đây, cả hai đường dẫn `/static/outputs/{task_id}.mp3` và `/outputs/{task_id}.mp3` đều truy cập chính xác tới tệp tin âm thanh đầu ra trên đĩa.

### 2. Khắc phục lỗi nuốt từ đầu dòng và từ đầu sau dấu câu (như "Mọi người", "Nhưng khi")
* **Nguyên nhân**: 
  1. Khi văn bản dài được tách câu, các đoạn text đầu vào được `strip()` sạch sẽ khoảng trắng. Do không có khoảng trống đệm ở đầu văn bản gửi tới mô hình OmniVoice (một mô hình autoregressive/diffusion), quá trình suy diễn (inference) ở những frame đầu tiên bị bất ổn định dẫn tới việc bỏ sót hoặc phát âm lỗi phụ âm đầu.
  2. Sự kết hợp giữa bộ xử lý lọc im lặng (`postprocess_output=True` của OmniVoice) và độ trễ phần cứng (hardware audio latency / fade-in của trình duyệt/máy phát) khiến các mili-giây âm thanh đầu tiên của mỗi đoạn hội thoại bị cắt cụt trực tiếp khi phát.
* **Khắc phục**:
  * Thực hiện cải tiến kép trong [omnivoice_lr.py](file:///e:/Document/TextToSpeech_Project/OmniVoice_TTS_Service_api/engines/omnivoice_lr.py) tại hàm `_synthesize_sync`:
    1. **Text Padding**: Tự động thêm khoảng trắng đệm vào đầu và cuối văn bản trước khi gọi mô hình (`processed_text = " " + text.strip() + " "`). Điều này giúp mô hình TTS có một khoảng đệm nhẹ để ổn định bộ giải mã (decoder) trước khi phát âm chữ đầu tiên.
    2. **Audio Physical Padding**: Chèn vật lý một mảng dữ liệu âm thanh im lặng (silence array) dài **100ms** (2400 samples ở tần số 24kHz) vào đầu mảng PCM và **50ms** (1200 samples) vào cuối mảng PCM trước khi đóng gói thành định dạng WAV.
  * Nhờ giải pháp cải tiến kép này, giọng đọc sinh ra cực kỳ tròn vành rõ chữ, hoàn toàn không bị nuốt âm hoặc mất từ đầu tiên của bất kỳ câu nào trong toàn bộ cuốn sách nói, đồng thời duy trì nhịp nghỉ ngơi chuyển câu rất tự nhiên.
